...
public class ServiciosQuerysArquetipo {
	...
	public String queryInsert(  String nombre, String apePaterno, String apeMaterno, String idUsuarioAlta) {

		return "INSERT INTO `SVC_PERSONA`(`NOM_PERSONA`, `NOM_PRIMER_APELLIDO`, `NOM_SEGUNDO_APELLIDO`, ID_USUARIO_ALTA) "
				+ " VALUES (\' #{nombre}\',\'#{apePaterno}\',\'#{apeMaterno}\' ,\'#{idUsuarioAlta}\')"
				.replace("#{nombre}", nombre).replace("#{apePaterno}", apePaterno).replace("#{apeMaterno}", apeMaterno).replace("#{idUsuarioAlta}", idUsuarioAlta);
